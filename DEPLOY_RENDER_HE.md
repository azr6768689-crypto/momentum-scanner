...
- `render.yaml` — Blueprint עם **`plan: free`** (דורש ריפו **Public** ב-GitHub).
...
**חינם ב־Render:** אפשר **רק** אם הריפו ב-GitHub הוא **Public**. ב־`render.yaml` מופיע **`plan: free`**. המפתחות (`POLYGON_API_KEY`, סיסמה) נשארים ב־Render בלבד — **לא** בקוד; ודא שאין `.env` בגיט. אם הריפו נשאר **Private** — Render דורש תוכנית בתשלום (Starter).
